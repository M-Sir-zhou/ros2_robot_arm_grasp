...
uart = serial.Serial('/dev/ttyACM0', uart_baudrate)  # 在 windows 下控制一体化关节，相应的输入连接的COM口和波特率
...
def get_ids():
    ...
    try:
        ...
        property = 'dr.config.can_node_id'
        address = property
...
def read_data_state2(n): # 16*n个字节
    ...
    byte_list_head = 0
    i = 500
    while uart.inWaiting() == 0 and i > 0:
        time.sleep(0.001)
        i -= 1
        if i < 495:
            order_num = 0x24
            data = format_data([order_num], 'u32', 'encode')
            send_command(id_num=0, cmd=0x08, data=data, rtr=0)
            i = 500
    while byte_list_head != 170:
        byte_list_head = uart.read(1)[0]
    byte_list.append(byte_list_head)
