...
#sudo chmod a+rw /dev/ttyACM0  # 临时赋予所有用户读写权限
id_num = 7



dr.set_angle_adaptive(id_num=id_num, angle=-5, speed=2, torque=0.4)#最闭合
# dr.set_angle_adaptive(id_num=id_num, angle=0, speed=10, torque=0.3)
# dr.set_angle_adaptive(id_num=id_num, angle=30, speed=10, torque=0.3)
...
'''单个关节自适应绝对角度控制'''
# dr.set_angle_adaptive(id_num=id_num, angle=360, speed=2, torque=2)
...
'''多个关节运动跟随与助力'''
# dr.motion_follow_aid_multi(id_list=id_list, angle_list=[60, 60], speed_list=[5, 3], angle_err_list=[1, 1], speed_err_list=[1, 0.1], torque_list=[3, 3])
...
'''多个关节力矩控制'''
# dr.set_torque(id_num=id_num, torque=-3, param=1, mode=1)
# time.sleep(4)
# dr.estop(id_num=id_num) # 急停
