...
def get_and_process_data(data_dir):
    ...
    #创建相机对象  相机的内外参  需要手动修改
    camera = CameraInfo(1280.0, 
                        720.0, 
                        intrinsic[0][0], 
                        intrinsic[1][1], 
                        intrinsic[0][2], 
    ...
    if len(cloud_masked) >= cfgs.num_point:
        # idxs = np.randomdemo.py.choice(len(cloud_masked), cfgs.num_point, replace=False)旧版本的numpy
...
def print_grasp_poses(gg):
    ...
    for i,grasp in enumerate(gg):
        print(f"\nGrasp{i+1}:")
        print(f"Postion(X,Y,Z):{grasp.translation}")
